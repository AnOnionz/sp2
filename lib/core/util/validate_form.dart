...
class ValidateForm {
  ...
  Future<Either<Failure, FormEntity>> validateForm(FormEntity form) async {
    // name
    if(form.customer.name.split('').where((element) {
    ...
    // phone
    if(form.customer.phoneNumber.length != 10 || !RegExp(r'^0[^01]([0-9]+)').hasMatch(form.customer.phoneNumber)){
      print('phone error');
      print(form.customer.phoneNumber.length );
